...
const ReportSearch = ({ navigation }) => {
  ...
  const onSearch = async () => {
    if (search.length < 2) {
      ...
      const response = await axios.get(`${key.API}/report/?search=${search}`);
      ...
      <ScrollView>
        <View style={styles.inner}>
          ...
          {reports.map((report, i) => (
            <ListItemFaq
              fullVisible={
                !report?.isSecret || userInfo?.is_admin ? true : false
              }
              ...
              onPress={() => {
                if (!report?.isSecret || userInfo?.is_admin) {
